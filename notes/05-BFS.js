...
function BFSBuildTree(arr) {
	...
	while (arr.length > 0) {
		...
		const lNode = arr.shift()
		if (lNode === null) continue
		let left = new TreeNode(lNode)
		node.left = left
		nodeQueue.push(left)
		// 每拼接一片叶子节点 重新判断剩余叶子数量 若无剩余视为拼接完毕
		if (!arr.length) break
		// 左侧叶子拼完，右边一样的操作
		const rNode = arr.shift()
		if (rNode === null) continue
		let right = new TreeNode(rNode)
		node.right = right
...
const testTree = [1, 2, null, 4, 5, 6, 7, 8, 9]
console.log(BFSBuildTree(testTree))
